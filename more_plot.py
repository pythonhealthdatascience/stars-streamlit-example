...
def ci_percentile(results, field, percentile, critical_value=1.96):
    ...
    Confidence intervals may or may not be symmetric.
    Critical value hard coded at the moment.
    Should update to use t dist.
    ...
    results: pd.DataFrame
        Results dataframe - tabular data where each row is a rep and each col
        is a KPI
    ...
    """
    half_width = critical_value * np.sqrt(
        (percentile * (1 - percentile)) / (len(results) - 1))
...
def more_plotly(
        data, x_label, percentiles=(0.05, 0.95), suppress_warnings=False):
    ...
    Error illustrated for CIs for mean and wide approx confidence intervals
    for percentiles.

    Confidence intervals for percentiles will only be calculated if > 80
    replications due to approximation accuracy.

    Notes:
    ------
    Each value plotted represents the mean of a replication (e.g. daily
    throughput). It should not be confused with an individuals results
    (e.g. an individuals throughput time).

    If the system modelled contains time dependency the MORE plot may hide
    time of day/event effects.
    ...
    suppress_warnings: bool, optional (default=False)
        Suppress warning that CIs for percentiles are not produced for
        < 80 replications.

    Returns:
    -------
        plotly.express.Figure: The Plotly figure object.
    ...
    WARN = f"""CIs for percentiles are not generated as
               sample size < {MIN_N_FOR_PERCENTILE}."""
    WARN += " To suppress this msg set `suppress_warnings=True`"

    # Calculate 5th and 95th percentiles and round up to the nearest integer
    ...
    fig.add_vrect(x0=lower_limit, x1=upper_limit, line_width=0,
                  fillcolor="#FFA726", opacity=0.5)  # Orange
